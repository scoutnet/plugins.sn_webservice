<?php
namespace ScoutNet\Api\Helpers;

/***************************************************************
 *
 *  Copyright notice
 *
 *  (c) 2017 Stefan "Mütze" Horst <muetze@scoutnet.de>, ScoutNet
 *
 *  All rights reserved
 *
 *  The GNU General Public License can be found at
 *  http://www.gnu.org/copyleft/gpl.html.
 *
 *  This script is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *  GNU General Public License for more details.
 *
 *  This copyright notice MUST APPEAR in all copies of the script!
 ***************************************************************/

use Exception;

class AesHelper {
    const AES_MODE_CBC = 'CBC';
    const AES_MODE_ECB = 'ECB';

    // The number of 32-bit words comprising the plaintext and columns comrising the state matrix of an AES cipher.
    private static $Nb = 4;
    // The number of 32-bit words comprising the cipher key in this AES cipher.
    private $Nk;
    // The number of rounds in this AES cipher.
    private $Nr;

    // The S-Box substitution table.
    private static $sBox = array(
        
    );

    // The inverse S-Box substitution table.
    private static $invSBox = array(
        
    );

    // Log table based on 0xe5
    private static $ltable = array(
        
    );

    // Inverse log table
    private static $atable = array(
        
    );

    // The key schedule in this AES cipher.
    private $w;
    // The state matrix in this AES cipher with Nb columns and 4 rows
    private $s;

    private $mode;
    private $iv = array();

    /**
     * constructs an AES cipher using a specific key.
     *
     * @param string $key
     * @param string $mode
     * @param string $iv
     *
     * @throws Exception
     */
    public function __construct($key, $mode = self::AES_MODE_ECB, $iv = '1234567890123456') {
        $this->Nk = strlen($key) / 4;
        $this->Nr = $this->Nk + self::$Nb + 2;

        if ($this->Nk != 4 && $this->Nk != 6 && $this->Nk != 8)
            throw new Exception("Key is " . ($this->Nk * 32) . " bits long. *not* 128, 192, or 256.");

        $this->Nr = $this->Nk + self::$Nb + 2;
        $this->w = array(); // Nb*(Nr+1) 32-bit words
        $this->s = array(array());  // 2-D array of Nb colums and 4 rows

        $this->KeyExpansion($key); // places expanded key in w
        $this->mode = $mode;

        $xsize = strlen($iv);
        for ($j = 0; $j < 16; $j++) {
            if (($j) < $xsize) {
                $this->iv[$j] = $iv[$j];
            } else
                $this->iv[$j] = chr(0);
        }
    }

    /**
     * Encrypts an aribtrary length String.
     *
     * @param string $plaintext
     * @return string cyphertext
     *
     * Whenever possible you should stream your plaintext through the
     * encryptBlock() function directly, as the amount of time required
     * to encrypt is linear to the size of the ciphertext.
     */
    public function encrypt($plaintext) {
        $t = ""; // 16-byte block
        $cyphertext = ""; // returned cipher text;

        // put a 16-byte block into t
        $xsize = strlen($plaintext);
        for ($i = 0; $i < $xsize; $i += 16) {
            for ($j = 0; $j < 16; $j++) {
                if (($i + $j) < $xsize) {
                    $t[$j] = $plaintext[$i + $j];
                } else
                    $t[$j] = chr(0);
            }

            $cyphertext .= $this->encryptBlock($t);

        }
        return $cyphertext;
    }

    /**
     * Decrypts an aribtrary length String.
     *
     * @param string $ciphertext
     * @returns string plaintext
     *
     *   Whenever possible you should stream your ciphertext through the
     *   decryptBlock() function directly, as the amount of time required
     *   to decrypt is linear to the size of the ciphertext.
     **/
    public function decrypt($ciphertext) {
        $t = ""; // 16-byte block
        $plaintext = ""; // returned plain text;

        // put a 16-byte block into t
        $ysize = strlen($ciphertext);
        for ($i = 0; $i < $ysize; $i += 16) {
            for ($j = 0; $j < 16; $j++) {
                if (($i + $j) < $ysize)
                    $t[$j] = $ciphertext[$i + $j];
                else
                    $t[$j] = chr(0);
            }

            $plaintext .= $this->decryptBlock($t);
        }
        return $plaintext;
    }

    /**
     * Encrypts the 16-byte plain text.
     *
     * @param string $plaintext_block 16-byte plaintext
     * @return string 16-byte ciphertext
     **/
    public function encryptBlock($plaintext_block) {
        $ciphertext_block = ""; // 16-byte string

        // place input into the initial state matrix in column order
        for ($i = 0; $i < 4 * self::$Nb; $i++) {
            if ($this->mode === self::AES_MODE_CBC) {
                $plaintext_block[$i] = chr(ord($this->iv[$i]) ^ ord($plaintext_block[$i]));
            }
            // we want integerger division for the second index
            $this->s[$i % 4][($i - $i % self::$Nb) / self::$Nb] = ord($plaintext_block[$i]);
        }

        // add round key
        $this->addRoundKey(0);

        for ($i = 1; $i < $this->Nr; $i++) {
            // substitute bytes
            $this->subBytes();

            // shift rows
            $this->shiftRows();

            // mix columns
            $this->mixColumns();

            // add round key
            $this->addRoundKey($i);
        }

        // substitute bytes
        $this->subBytes();

        // shift rows
        $this->shiftRows();

        // add round key
        $this->addRoundKey($i);


        // place state matrix s into ciphertext in column order
        for ($i = 0; $i < 4 * self::$Nb; $i++) {
            $enc = chr($this->s[$i % 4][($i - $i % self::$Nb) / self::$Nb]);
            if ($this->mode === self::AES_MODE_CBC) {
                $this->iv[$i] = $enc;
            }

            $ciphertext_block .= $enc;
        }
        return $ciphertext_block;
    }

    /**
     * Decrypts the 16-byte cipher text.
     *
     * @param string $ciphertext_block 16-byte ciphertext
     * @return string 16-byte plaintext
     **/
    public function decryptBlock($ciphertext_block) {
        $plaintext_block = ""; // 16-byte string


        // place input into the initial state matrix in column order
        for ($i = 0; $i < 4 * self::$Nb; $i++)
            $this->s[$i % 4][($i - $i % self::$Nb) / self::$Nb] = ord($ciphertext_block[$i]);

        // add round key
        $this->addRoundKey($this->Nr);

        for ($i = $this->Nr - 1; $i > 0; $i--) {
            // inverse shift rows
            $this->invShiftRows();

            // inverse sub bytes
            $this->invSubBytes();

            // add round key
            $this->addRoundKey($i);

            // inverse mix columns
            $this->invMixColumns();
        }

        // inverse shift rows
        $this->invShiftRows();

        // inverse sub bytes
        $this->invSubBytes();

        // add round key
        $this->addRoundKey($i);

        // place state matrix s into plaintext_block in column order
        for ($i = 0; $i < 4 * self::$Nb; $i++) {
            $dec = chr($this->s[$i % 4][($i - $i % self::$Nb) / self::$Nb]);

            if ($this->mode === self::AES_MODE_CBC) {
                $dec = chr(ord($this->iv[$i]) ^ ord($dec));
                $this->iv[$i] = $ciphertext_block[$i];
            }

            // Used to remove filled null characters.
            $plaintext_block .= ($dec == chr(0) ? "" : $dec);
        }

        return $plaintext_block;
    }

    public function __destruct() {
        unset($this->w);
        unset($this->s);
    }

    /**
     * makes a big key out of a small one
     *
     * @param string $key input
     * @return void
     */
    private function KeyExpansion($key) {
        // Rcon is the round constant
        static $Rcon = array(
            0x00000000,
            0x01000000,
            0x02000000,
            0x04000000,
            0x08000000,
            0x10000000,
            0x20000000,
            0x40000000,
            0x80000000,
            0x1b000000,
            0x36000000,
            0x6c000000,
            0xd8000000,
            0xab000000,
            0x4d000000,
            0x9a000000,
            0x2f000000
        );

        // the first Nk words of w are the cipher key z
        for ($i = 0; $i < $this->Nk; $i++) {
            $this->w[$i] = 0;
            // fill an entire word of expanded key w
            // by pushing 4 bytes into the w[i] word
            $this->w[$i] = ord($key[4 * $i]); // add a byte in
            $this->w[$i] <<= 8; // make room for the next byte
            $this->w[$i] += ord($key[4 * $i + 1]);
            $this->w[$i] <<= 8;
            $this->w[$i] += ord($key[4 * $i + 2]);
            $this->w[$i] <<= 8;
            $this->w[$i] += ord($key[4 * $i + 3]);
        }


        for (; $i < self::$Nb * ($this->Nr + 1); $i++) {
            $temp = $this->w[$i - 1];

            if ($i % $this->Nk == 0)
                $temp = $this->subWord($this->rotWord($temp)) ^ $Rcon[$i / $this->Nk];
            else if ($this->Nk > 6 && $i % $this->Nk == 4)
                $temp = $this->subWord($temp);

            $this->w[$i] = $this->w[$i - $this->Nk] ^ $temp;

            self::make32BitWord($this->w[$i]);
        }
    }

    /**
     * adds the key schedule for a round to a state matrix.
     *
     * @param $round
     * @return void
     */
    private function addRoundKey($round) {
        for ($i = 0; $i < 4; $i++) {
            for ($j = 0; $j < self::$Nb; $j++) {
                // place the i-th byte of the j-th word from expanded key w into temp
                $temp = $this->w[$round * self::$Nb + $j] >> (3 - $i) * 8;
                // Cast temp from a 32-bit word into an 8-bit byte.
                $temp %= 256;
                // Can't do unsigned shifts, so we need to make this temp positive
                $temp = ($temp < 0 ? (256 + $temp) : $temp);

                $this->s[$i][$j] ^= $temp; // xor temp with the byte at location (i,j) of the state
            }
        }
    }

    /**
     * unmixes each column of a state matrix.
     *
     * @return void
     **/
    private function invMixColumns() {
        // There are Nb columns
        for ($i = 0; $i < self::$Nb; $i++) {
            $s0 = $this->s[0][$i];
            $s1 = $this->s[1][$i];
            $s2 = $this->s[2][$i];
            $s3 = $this->s[3][$i];

            $this->s[0][$i] = $this->mult(0x0e, $s0) ^ $this->mult(0x0b, $s1) ^ $this->mult(0x0d, $s2) ^ $this->mult(0x09, $s3);
            $this->s[1][$i] = $this->mult(0x09, $s0) ^ $this->mult(0x0e, $s1) ^ $this->mult(0x0b, $s2) ^ $this->mult(0x0d, $s3);
            $this->s[2][$i] = $this->mult(0x0d, $s0) ^ $this->mult(0x09, $s1) ^ $this->mult(0x0e, $s2) ^ $this->mult(0x0b, $s3);
            $this->s[3][$i] = $this->mult(0x0b, $s0) ^ $this->mult(0x0d, $s1) ^ $this->mult(0x09, $s2) ^ $this->mult(0x0e, $s3);
        }
    }

    /**
     * applies an inverse cyclic shift to the last 3 rows of a state matrix.
     *
     * @return void
     **/
    private function invShiftRows() {
        $temp = "";
        for ($i = 1; $i < 4; $i++) {
            for ($j = 0; $j < self::$Nb; $j++)
                $temp[($i + $j) % self::$Nb] = $this->s[$i][$j];
            for ($j = 0; $j < self::$Nb; $j++)
                $this->s[$i][$j] = $temp[$j];
        }
    }

    /**
     * applies inverse S-Box substitution to each byte of a state matrix.
     *
     * @returns void
     **/
    private function invSubBytes() {
        for ($i = 0; $i < 4; $i++)
            for ($j = 0; $j < self::$Nb; $j++)
                $this->s[$i][$j] = self::$invSBox[$this->s[$i][$j]];
    }

    /**
     * mixes each column of a state matrix.
     *
     * @returns void
     **/
    private function mixColumns() {
        // There are Nb columns
        for ($i = 0; $i < self::$Nb; $i++) {
            $s0 = $this->s[0][$i];
            $s1 = $this->s[1][$i];
            $s2 = $this->s[2][$i];
            $s3 = $this->s[3][$i];

            $this->s[0][$i] = $this->mult(0x02, $s0) ^ $this->mult(0x03, $s1) ^ $this->mult(0x01, $s2) ^ $this->mult(0x01, $s3);
            $this->s[1][$i] = $this->mult(0x01, $s0) ^ $this->mult(0x02, $s1) ^ $this->mult(0x03, $s2) ^ $this->mult(0x01, $s3);
            $this->s[2][$i] = $this->mult(0x01, $s0) ^ $this->mult(0x01, $s1) ^ $this->mult(0x02, $s2) ^ $this->mult(0x03, $s3);
            $this->s[3][$i] = $this->mult(0x03, $s0) ^ $this->mult(0x01, $s1) ^ $this->mult(0x01, $s2) ^ $this->mult(0x02, $s3);
        }
    }

    /**
     * applies a cyclic shift to the last 3 rows of a state matrix.
     *
     * @return void
     **/
    private function shiftRows() {
        $temp = "";
        for ($i = 1; $i < 4; $i++) {
            for ($j = 0; $j < self::$Nb; $j++)
                $temp[$j] = $this->s[$i][($j + $i) % self::$Nb];
            for ($j = 0; $j < self::$Nb; $j++)
                $this->s[$i][$j] = $temp[$j];
        }
    }

    /**
     * applies S-Box substitution to each byte of a state matrix.
     *
     * @return void
     **/
    private function subBytes() {

        for ($i = 0; $i < 4; $i++) {
            for ($j = 0; $j < self::$Nb; $j++)
                $this->s[$i][$j] = self::$sBox[$this->s[$i][$j]];
        }
    }

    /**
     * multiplies two polynomials a(x), b(x) in GF(2^8) modulo the irreducible polynomial m(x) = x^8+x^4+x^3+x+1
     *
     * @param $a
     * @param $b
     * @return string 8-bit value
     */
    private static function mult($a, $b) {
        $sum = self::$ltable[$a] + self::$ltable[$b];
        $sum %= 255;
        // Get the antilog
        $sum = self::$atable[$sum];
        return ($a == 0 ? 0 : ($b == 0 ? 0 : $sum));
    }

    /**
     * applies a cyclic permutation to a 4-byte word.
     *
     * @param string $word
     * @return int 32-bit
     */
    private static function rotWord($word) {
        $temp = $word >> 24; // put the first 8-bits into temp
        $word <<= 8; // make room for temp to fill the lower end of the word
        self::make32BitWord($word);
        // Can't do unsigned shifts, so we need to make this temp positive
        $temp = ($temp < 0 ? (256 + $temp) : $temp);
        $word += $temp;

        return $word;
    }

    /**
     * applies S-box substitution to each byte of a 4-byte word.
     *
     * @param string $word
     * @return int 32-bit
     **/
    private static function subWord($word) {
        // loop through 4 bytes of a word
        for ($i = 0; $i < 4; $i++) {
            $temp = $word >> 24; // put the first 8-bits into temp
            // Can't do unsigned shifts, so we need to make this temp positive
            $temp = ($temp < 0 ? (256 + $temp) : $temp);
            $word <<= 8; // make room for the substituted byte in w;
            self::make32BitWord($word);
            $word += self::$sBox[$temp]; // add the substituted byte back
        }

        self::make32BitWord($word);

        return $word;
    }

    /**
     * reduces a 64-bit word to a 32-bit word
     *
     * @param string $word
     * @return void
     **/
    private static function make32BitWord(&$word) {
        // Reduce this 64-bit word to 32-bits on 64-bit machines
        $word &= 0x00000000FFFFFFFF;
    }
}
