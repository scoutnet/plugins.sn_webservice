        ...
        stage('Test'){
            steps {
                ...
                withCredentials([string(credentialsId: 'CODECOV_TOKEN', variable: 'CODECOV_TOKEN')]) {
                    bash 'ci_env=`bash <(curl -s https://codecov.io/env)` && docker run --rm $ci_env -w /opt/data -v `pwd`:/opt/data -i epcallan/php7-testing-phpunit:7.0-phpunit6 bash -c "bash <(curl -s https://codecov.io/bash)"'
                }
            }
        }
        stage('Build'){
