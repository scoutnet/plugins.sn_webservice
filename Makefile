...
$(TEST_ROOT_FOLDER)/docker-%/.env:
	...
	@echo "TEST_FILE=`pwd`/Tests/$(shell php -r 'echo ucfirst("$(word 2,$(subst -, ,$*))");')" >> $(TEST_ROOT_FOLDER)/docker-$*/.env
...
.PHONY: test
test: lintTest cglTest unitTest
	@echo "done"

# Availbable Tests
ALL_TESTS = $(foreach ver,$(subst .,,$(PHP_VERSIONS)),test-php$(ver))
.PHONY: $(ALL_TESTS)
$(ALL_TESTS): test-php%: lintTest-php% cglTest-php% unitTest-php%
